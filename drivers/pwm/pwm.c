...
uint16_t pwm_to_pin[] = {
    [PWM_MOTOR1] = PWM_MOTOR1_PIN,
    [PWM_MOTOR2] = PWM_MOTOR1_PIN,
    [PWM_MOTOR3] = PWM_MOTOR1_PIN,
    [PWM_MOTOR4] = PWM_MOTOR1_PIN,
    [PWM_ALL] = (PWM_MOTOR1_PIN | PWM_MOTOR2_PIN | PWM_MOTOR3_PIN | PWM_MOTOR4_PIN),
};
#if 0
...
static void init_pin_for_pwm_use(enum pwm pwm)
{

    uint16_t pin = pwm_to_pin[pwm];
...
void TM_LEDS_Init(void) {
	...
	GPIO_PinAFConfig(GPIOD, pwm_to_pin[PWM_MOTOR1], GPIO_AF_TIM4);
	...
	/* Set pins */
	GPIO_InitStruct.GPIO_Pin = pwm_to_pin[PWM_ALL];
...
void pwm_set_percent(uint8_t pct)
{
	...
	TIM_OCStruct.TIM_Pulse = new_value; /* 75% duty cycle */
	TIM_OC3Init(TIM4, &TIM_OCStruct);
	TIM_OC3PreloadConfig(TIM4, TIM_OCPreload_Enable);
#endif
}
